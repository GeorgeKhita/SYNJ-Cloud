...
export async function provision(serviceId, { email, firstName, productType, templateId, resources, cartId, externalOrderId, paymentIntentId }) {
  ...
  try {
    ...
    // Tente de nettoyer le CT orphelin si vmId a été obtenu
    if (vmId) {
      proxmox.stopContainer(node, vmId).catch(() => {});
      proxmox.deleteContainer(node, vmId).catch(() => {});
    }
    ...
    if (paymentIntentId) {
      stripe.refund(paymentIntentId, err.message)
        .catch(() => logger.error({ serviceId, paymentIntentId }, 'stripe:refund_unhandled'));
    }
